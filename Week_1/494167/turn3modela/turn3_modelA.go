...
func verifyToken(r *http.Request) (string, error) {
	...
	if subject, ok := claims["sub"]; ok && token.Valid {
...
func rateLimitCheck(ip string) bool {
	val, err := red.Get(ip).Result()
	if val == "" {
		success, err := red.Set(ip, fmt.Sprintf("%d", 1), time.Minute*5).Result()
		if success != "OK" {
			fmt.Println(err)
			return true
		}
...
func recordFailedAttempt(ip string) {
	res, err := red.Incr(ip).Result()
	if err != nil {
		log.Println(err)
	}
	err = red.Set(ip, fmt.Sprintf("%d", 1+res), time.Minute*5).Err()
	if err != nil {
		log.Println(err)
	}
}
...
func authenticateUser(email, password string) (User, error) {
	...
	if email == "john.doe@example.com" && (password == "secret" || (password == "" && refreshToken != "")) {
